...
class DataLoader:
    def __init__(self, data_folder=r"D:\DATA\Airbus Ship Detection Challenge\airbus-ship-detection\data"):
        """
        ...
        self.test_images_path = data_folder + r"\\" + r"images\test"
        self.train_images_path = data_folder + r"\\" + r"images\train"

        train_images = set(os.listdir(self.train_images_path)) - deformed
        test_images = set(os.listdir(self.test_images_path)) - deformed
        # ship_masks_path = data_folder + r"\\" + "train_ship_segmentations_v2.csv"
        all_masks = pd.read_csv('train_ship_segmentations_v2.csv')
        ...
        train_encodedPixelsDF = (
            train_masks[["ImageId", "EncodedPixels"]]
        ...
        self.train["has_ship"] = self.train["ships"].map(
        ...
        test_masks["ships"] = test_masks["EncodedPixels"].map(
            lambda c_row: 1 if isinstance(c_row, str) else 0
        )
        test_encodedPixelsDF = (
            test_masks[["ImageId", "EncodedPixels"]]
            .groupby("ImageId")["EncodedPixels"]
            .apply(list)
        )
    ...
    def get_masks(self, image_id, from_train=True):
        ...
        img_masks = None
        if from_train:
            img_masks = self.train.loc[
                self.train["ImageId"] == image_id, "EncodedPixels"
            ].values[0]
        else:
    ...
    def get_grouped_images_gen(
        self, df=None, batch_size=8, img_scaling=(3, 3), f=None, train=True
    ):
        """
    ...
    def train_split(self, valid_size=0.3, adjust_set=True, filterNan=False):
        ...
        train_df, valid_df = train_test_split(
            df, test_size=valid_size, stratify=df["ships"]
        )  # the stratified class must be "ships"
    ...
    def get_image(self, img_id, from_train=True):
        ...
        if from_train:
            ...
            return imread(os.path.join(self.train_images_path, str(img_id)))
        else:

            return imread(os.path.join(self.test_images_path, str(img_id)))
