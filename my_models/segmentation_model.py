...
class SegmentationModel:
    ...
    def dice_p_bce(self, in_gt, in_pred):
        """ Loss Function"""
        return 1e-3 * binary_crossentropy(in_gt, in_pred) - self.dice_coef(
            in_gt, in_pred
        )
    ...
    def compile(self):
        """
        compiling my_models, using DICE and IoU as main metrics, and ADAM optimizer.
        """
        self.seg_model.compile(
            optimizer=Adam(1e-3, decay=1e-6),
            loss=self.dice_p_bce,
            metrics=[self.dice_coef, self.IoU, "binary_accuracy"],
        )
    ...
    def train(self, gen, input_len, valid_set, epochs=5, train_steps=-1, batch_size=9):
        ...
        valid_x = valid_set[0].astype(np.float32)
        valid_y = valid_set[1].astype(np.float32)
        if train_steps > 0:
    ...
    def show_loss(self, loss_history):
        ...
        for i in range(len(keys)):
            if "val_" + keys[i] in loss_history[0].history.keys():
                _ = axn[i].plot(
                    epochs,
                    np.concatenate([mh.history[keys[i]] for mh in loss_history]),
                    "b-",
                    epochs,
                    np.concatenate(
                        [mh.history["val_" + keys[i]] for mh in loss_history]
                    ),
                    "r-",
                )
                ...
                axn[i].set_title(keys[i])
        fig.savefig(self.model_folder + "/" + self.name + "_loss.jpg")
    ...
    def examine_performance(self, test_x, text_y, n=40, load=False):
        ...
        plt.show()
        fig.savefig(self.model_folder + "/" + self.name + "_res.jpg")
